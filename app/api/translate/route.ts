...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // 输入校验
    const userContent = String(body.content || '').slice(0, MAX_CONTENT_LENGTH);
    ...
    // 彩蛋：木生 → 淑柔，25% 概率融入木棉花意象
    if (senderName.includes('木生') && receiverName.includes('淑柔') && Math.random() < 0.25) {
      userMsg += '\n提示：可用木棉花（英雄花）比喻你们之间炽热坚贞的情感，像木棉一样即使飘落也鲜红如初。';
    }

    userMsg += '\n注意：只分享近况和表达思念，不教导、不承诺，不过度过问（如家中长辈身体如何？小孩有没有努力读书等。因为通常来信会主动说，就不需要过问）。';
    ...
    // 彩蛋：收信人含"晓英"，25%概率附上暹罗情话
    if (receiverName.includes('晓英') && Math.random() < 0.99) {
      content += '暹罗没有春天，你就是我的春天。';
    }
